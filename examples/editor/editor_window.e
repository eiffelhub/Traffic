	...
	set_city (a_city: CITY)
		...
		do
			...
			if city = Void then
				...
				across city.stations as l_stations loop
					map.station_view (l_stations.item).on_left_click_no_args.extend_back (agent set_selected_station (l_stations.item))
					map.station_view (l_stations.item).on_double_click_no_args.extend_back (agent set_selected_station (l_stations.item))
...
--					l_stations.value.mouse_double_clicked_actions.force_extend (agent center_map_on_station (l_stations.value.station))
				...
				across city.lines as l_lines loop
					map.line_view (l_lines.item).on_left_click_no_args.extend_back (agent set_selected_line (l_lines.item))
--					l_lines.value.mouse_clicked_actions.force_extend (agent set_selected_line (l_lines.value.line))
	...
	set_selected_station (a_station: STATION)
		...
		do
			...
			map.station_view (a_station).highlight
...
--			map.projector.project
	...
	set_selected_line (a_line: LINE)
		...
		do
			...
			map.line_view (a_line).highlight
...
--			map.projector.project
	...
	deselect_line
		...
		do
			...
			across city.lines as i loop
				map.line_view (i.item).unhighlight
			end
			across line_list as i loop
				i.item.disable_select
			end
	...
	on_station_selected
		...
		do
			if attached {STRING} station_list.selected_item.data as l_station_name then
				set_selected_station (map.station_view (city.station (l_station_name)).model)
			end
		end
	...
	on_line_selected
		...
		do
			if attached {INTEGER} line_list.selected_item.data as l_line_number then
				set_selected_line (map.line_view (city.line (l_line_number)).model)
			end
		end
